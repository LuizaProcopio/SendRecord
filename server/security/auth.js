...
class AuthenticationSystem {
  ...
  async isAccountLocked(email) {
    ...
    try {
      ...
      return rows[0].failed_attempts >= this.MAX_LOGIN_ATTEMPTS;
    } catch (error) {
  ...
  async login(email, password, ipAddress) {
    try {
      ...
      const [users] = await conexao.query(query, [email]);
      ...
      await this.logAuditoria(user.id, 'LOGIN', 'Usuário autenticado com sucesso', null, null, ipAddress);
